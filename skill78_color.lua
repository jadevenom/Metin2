			...
			if confirm == 1 then
				...
				if pc.learn_grand_master_skill(vnum) then
					if pc.get_skill_level(vnum) == 40 then
						say("Tebrikler! ")
						say_color("green",string.format("%s perfect Master Oldu.", name))
						say("")
					else
						say("Tebrikler! ")
						say_color("green",string.format("Becerin yükseltildi %s G%d.", name, level-30+1+1))
						say("")
					end
					
				else
					pc.change_alignment(-number(need_alignment / 3, need_alignment / 2))
					say_color("red","Becerini geliþtiremedin! ")
